...
(defn parse [s]
  (into []
        (map (fn [l]
               (map (fn [instr]
                      (let [[_ dir steps] (re-find #"([UDLR])(\d+)" instr)]
                        {:dir (first dir)
                         :steps (Long/parseLong steps)}))
                    (str/split l #","))))
        (str/split-lines s)))

(defn moves [[x y :as pnt] {d :dir
                            s :steps}]
  (let [id identity
        mv {\U [id inc]
            \D [id dec]
...
(defn all-points-in-path [start path]
  (second (reduce (fn [[pnt points] instr]
                    (let [mvs (moves pnt instr)]
                      [(peek mvs) (into points mvs)]))
                  [start []]
                  path)))
    ...
    (apply min
           (map (partial taxicab-distance origin)
                (apply set/intersection
                       (map (comp set (partial all-points-in-path origin))
                            (parse paths)))))))
(defn part-2 [paths]
  (let [ixn->dist (fn [ixn pnts]
                    (reduce (fn [acc [idx p]]
                              (if (and (contains? ixn p)
                                       (not (contains? acc p)))
                                (assoc acc p (inc idx))
                                acc))
                            {}
