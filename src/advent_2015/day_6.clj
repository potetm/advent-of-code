...
(defn parse [in]
  (map (fn [s]
         (let [[_ command & r] (re-find #"(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)"
                                        s)
               [x1 y1 x2 y2] (map #(Long/parseLong %)
                                  r)]
           {:command command
            :point-1 [x1 y1]
            :point-2 [x2 y2]}))
...
(defn process [board commands instrs]
  (reduce (fn [b
               {c :command
                p1 :point-1
                p2 :point-2}]
...
(defn part-1 [in]
  (count (filter #{:on}
                 (board-vals (process (init :off)
                                      {"turn on" (constantly :on)
                                       "turn off" (constantly :off)
                                       "toggle" {:on :off
                                                 :off :on}}
                                      in)))))
...
(defn part-2 [in]
  (util/sum (board-vals (process (init 0)
                                 {"turn on" inc
                                  "turn off" #(max (dec %) 0)
                                  "toggle" #(+ 2 %)}
                                 in))))
